...
public class aoc2020_day4u : MonoBehaviour
{
...
#region //test_input
	string test_input = @"ecl:gry pid:860033327 eyr:2020 hcl:#fffffd
...
#endregion
#region //test_input_valid
	string test_input_valid = @"pid:087499704 hgt:74in ecl:grn iyr:2012 eyr:2030 byr:1980
hcl:#623a2f
...
#endregion
#region //input_data
	string input_data = @"byr:2024 iyr:2016
...
byr:1959";
		...
		string[] raw_arr = input_data.Split('\n');
		//getting length for input_array
		int blank_count = 0;
		for (int i = 0; i < raw_arr.Length; i++)
		{
			if (raw_arr[i].Length == 1)
			{
				blank_count++;
			}
		}
		//each passport is splitted by one blank line so total passport entries = total blank lines + 1
		string[] input_arr = new string[blank_count+1];
		//removing blank line in data and combining each passport entry.
		int a = 0;
		for (int i = 0 ; i < raw_arr.Length; i++)
		{
			if (raw_arr[i].Length > 1)
			{
				if (string.IsNullOrEmpty(input_arr[a]) == true) 
				{
					input_arr[a] = raw_arr[i];
				}
				else if (string.IsNullOrEmpty(input_arr[a]) == false)
				{
					input_arr[a] = input_arr[a] + ' ' + raw_arr[i];
				}			
			}
			else if (raw_arr[i].Length == 1)
			{
				a++;
			}
		}
		//finally part 1 answer!
		int part1_counter = 0;
		for (int i = 0; i < input_arr.Length; i++)
		{
			...
			{	
				if (input_arr[i].IndexOf(required_fields[j]) > -1)
				{
					field_counter++;
				}
				if (field_counter == required_fields.Length)
				{
					part1_counter++;
				}
			}
		}
		Debug.Log("Day 4 part 1 answer is: "+part1_counter);

		//part 2
		int part2_counter = 0;
		string[] valid_ecl = {"amb","blu","brn","gry","grn","hzl","oth"};

		for (int i = 0; i < input_arr.Length; i++)
		{
			int field_counter = 0;
			string[] passport_data = input_arr[i].Replace("\n","").Replace("\r","").Split(' ');

			if (passport_data.Length >= 7)
			{
				for (int j = 0; j < passport_data.Length; j++)
				{	
					string[] temp = new string[2];
					switch (passport_data[j].Substring(0,3))
					{
						case "byr":
							temp = passport_data[j].Split(':');
							int byr = int.Parse(temp[1]);
						...
						case "hgt":
							temp = passport_data[j].Split(':');
							string hgt = temp[1];
							if (hgt.IndexOf("cm") > -1 && hgt.Length == 5)
							{
								int hgt_val = int.Parse(hgt.Substring(0,3));
								if (hgt_val >= 150 || hgt_val <= 193)
								{
									field_counter++;
								}
							}
							else if (hgt.IndexOf("in") > -1 && hgt.Length == 4)
							{
								int hgt_val = int.Parse(hgt.Substring(0,2));
						...
						case "hcl":
							temp = passport_data[j].Split(':');
							string hcl = temp[1];
							if (hcl.Length == 7 && hcl.Substring(0,1) == "#")
							{	
								Color tempcol;
								if (ColorUtility.TryParseHtmlString(hcl, out tempcol))
								{
									field_counter++;
								}
							}
						...
						case "ecl":
							temp = passport_data[j].Split(':');
							string ecl = temp[1];
							for (int x = 0; x < valid_ecl.Length; x++)
							{
								if (ecl == valid_ecl[x])
								{
									field_counter++;
		...
		Debug.Log("Day 4 Part 2 answer is: "+ part2_counter);
	}
}
