...
public class aoc2020_day1u : MonoBehaviour {
	...
	void Start () {
		...
		for (int i = 0; i < input_arr.Length; i++)
		{
			for (int j = 0; j < input_arr2.Length; j++)
			{
				for (int k = 0; k < input_arr3.Length; k++)
				{
					if (input_arr[i] + input_arr2[j] + input_arr3[k] == 2020)
					{
						Debug.Log(input_arr[i]*input_arr2[j]*input_arr3[k]);
						return;
					}
				} 	
			}
