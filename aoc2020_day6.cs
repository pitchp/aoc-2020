...
yvfta
atypkqfv";
			string[] raw_arr = day6_input.Split('\n');
			//Console.WriteLine(string.IsNullOrEmpty(raw_arr[2]));
			int blank_count = 0;
			for (int i = 0; i < raw_arr.Length; i++)
			{
				if (string.IsNullOrEmpty(raw_arr[i]) == true)
				{
					blank_count++;
				}
			}
			//each group is splitted by one blank line so total groups = total blank lines + 1
			string[] input_arr = new string[blank_count+1];
			//removing blank line in data and combining each group's answers.
			int a = 0;
			for (int i = 0 ; i < raw_arr.Length; i++)
			{
				if (string.IsNullOrEmpty(raw_arr[i]) == false)
				{
				...
				else if (string.IsNullOrEmpty(raw_arr[i]) == true)
				{
					a++;
				}
			}
			...
			for (int i=0;i< input_arr.Length; i++)
			{
				...
				int group_answer = temp.Distinct().ToArray().Length; 
				p1_answer_sum += group_answer;
			}
			Console.WriteLine("Day 6 part 1 answer is: " + p1_answer_sum);
			
			//day 6 part 2 
			
			int p2_answer = 0;
			string[] group_data = day6_input.Split("\n\n");
			int[] group_ppl_count = new int[group_data.Length];
			for(int i=0;i<group_data.Length;i++)
			{
				group_ppl_count[i] = group_data[i].Split('\n').Length;
			}
			...
			{
				char[] temp = input_arr[i].ToCharArray();
				var question_dict = new Dictionary<char, int>();
				for (int j=0; j<temp.Length;j++)
				{
					if (question_dict.ContainsKey(temp[j]))
					{
						question_dict[temp[j]]++;
						
					}
					else
					{
						question_dict.Add(temp[j],1);
					}
				}
				//loop dict
				foreach (var answer in question_dict)
				{
					if (answer.Value == group_ppl_count[i])
					{
						p2_answer++;
					}
				}
			}
			Console.WriteLine("Day 6 part 2 answer is: " + p2_answer);
